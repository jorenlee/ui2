<template>
  <div :class="['p-4 min-h-screen', darkMode ? 'bg-gray-900 text-white' : 'bg-white text-gray-900']">
    <div class="flex gap-4">
      <!-- Sidebar -->
      <aside :class="['w-80 p-3 rounded shadow', darkMode ? 'bg-gray-800' : 'bg-gray-50']">
        <div class="flex items-center justify-between mb-3">
          <h3 class="font-bold">Upcoming Bookings</h3>
          <div class="flex items-center gap-2">
            <button @click="toggleSidebar" class="px-2 py-1 border rounded">{{ sidebarCollapsed ? 'Open' : 'Collapse' }}</button>
            <button @click="toggleDarkMode" class="px-2 py-1 border rounded">{{ darkMode ? 'Light' : 'Dark' }}</button>
          </div>
        </div>

        <div v-if="sidebarCollapsed" class="text-sm text-gray-500">Sidebar collapsed</div>

        <div v-else class="space-y-2 max-h-[70vh] overflow-auto">
          <div v-if="!schedules.length" class="text-sm text-gray-500">No bookings</div>
          <div v-for="s in upcoming" :key="s.id" class="p-2 rounded border" :class="darkMode ? 'bg-gray-700 border-gray-600' : 'bg-white'">
            <div class="flex justify-between items-start">
              <div>
                <div class="font-semibold">{{ s.notes || 'Schedule' }}</div>
                <div class="text-xs">{{ formatApiDate(s.date) }}</div>
                <div class="text-xs mt-1">Times: <span class="font-mono text-xs">{{ readableTimes(s.time) }}</span></div>
                <div v-if="s.recurrence" class="text-xs text-yellow-400 mt-1">Repeats: {{ s.recurrence }}</div>
              </div>
              <div class="text-right">
                <button @click="openEditFromSidebar(s)" class="px-2 py-1 border rounded text-sm">Edit</button>
              </div>
            </div>
          </div>
        </div>
      ...
      <div class="flex-1">
        ...
        <div class="flex items-center justify-between mb-4">
          <h2 class="text-xl font-bold text-green-800">Library Schedules — Calendar</h2>
          <div class="flex gap-2">
            <button @click="openCreateFromButton" class="bg-green-800 text-white px-4 py-2 rounded">New</button>
            <button @click="refresh" class="bg-white border px-3 py-2 rounded">Refresh</button>
          </div>
        </div>

        <!-- Toast -->
        <div v-if="toast.show" :class="['p-3 rounded mb-3', toast.type === 'error' ? 'bg-red-100 text-red-800' : 'bg-green-100 text-green-800']">
          {{ toast.message }}
        </div>
        ...
        <transition name="fade">
          <div v-if="showModal" class="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-40">
            <div :class="['rounded-lg w-full max-w-2xl p-6', darkMode ? 'bg-gray-800 text-white' : 'bg-white text-gray-900']">
              <h3 class="font-bold mb-4">{{ isEditing ? 'Edit Schedule' : 'Create New Schedule' }}</h3>
              ...
              <div class="grid gap-3">
               ...
               <label v-for="(t,i) in timeOptions" :key="i" class="flex items-center gap-2 text-sm">
  <input
    type="checkbox"
    :value="t._12_hour_format_from + ' - ' + t._12_hour_format_to"
    v-model="form.times"
  />
  <span>{{ t._12_hour_format_from }} - {{ t._12_hour_format_to }}</span>
</label>

                <label class="text-sm">Notes</label>
                <textarea v-model="form.notes" class="border p-2 rounded h-24"></textarea>

                <label class="inline-flex items-center gap-2 mt-2">
                  <input type="checkbox" v-model="form.recurring" />
                  <span class="text-sm">Recurring weekly</span>
                </label>

                <div v-if="form.recurring" class="grid grid-cols-7 gap-2 text-xs">
                  <label v-for="(d, idx) in weekDays" :key="d" class="flex flex-col items-center">
                ...
                <div class="flex justify-end gap-2 mt-3">
                  ...
                  <button v-if="!isEditing" @click="createSchedule" :disabled="isSubmitting" class="px-4 py-2 bg-green-800 text-white rounded">
                    {{ isSubmitting ? 'Saving...' : 'Create' }}
                  </button>

                  <div v-else class="flex gap-2">
                    <button @click="updateSchedule" :disabled="isSubmitting" class="px-4 py-2 bg-yellow-500 text-white rounded">
        ...
        <transition name="fade">
          <div v-if="showDeleteConfirm" class="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-40">
            <div :class="['rounded-lg w-full max-w-md p-6 text-center', darkMode ? 'bg-gray-800 text-white' : 'bg-white']">
              <h3 class="font-bold mb-4">Confirm Remove</h3>
              <p class="text-sm text-gray-700 mb-6">Remove schedule for <strong>{{ form.date }}</strong>?</p>
              <div class="flex justify-center gap-3">
                <button @click="showDeleteConfirm=false" class="px-4 py-2 border rounded">Cancel</button>
                <button @click="deleteSchedule" :disabled="isSubmitting" class="px-4 py-2 bg-red-700 text-white rounded">{{ isSubmitting ? 'Removing...' : 'Remove' }}</button>
              </div>
            </div>
          </div>
        </transition>
...
const formatApiDate = (d) => moment(d, ['MM-DD-YYYY','YYYY-MM-DD']).format('MMMM D, YYYY')
const readableTimes = (t) => {
  if (!t) return '—'
  if (Array.isArray(t)) return t.map(x => (typeof x === 'string' ? x : (x._12_hour_format_from + ' - ' + x._12_hour_format_to))).join(', ')
  if (typeof t === 'string') return t
  if (typeof t === 'object' && t._12_hour_format_from) return `${t._12_hour_format_from} - ${t._12_hour_format_to}`
  return String(t)
}
...
const normalizeTimeLabel = (t) => {
  ...
  if (typeof t === 'object') {
    if (t._12_hour_format_from && t._12_hour_format_to) return `${t._12_hour_format_from} - ${t._12_hour_format_to}`
    if (t.range_from_time && t.range_to_time) return `${t.range_from_time} - ${t.range_to_time}`
  }
  return null
}
...
const parseScheduleToEvents = (raw) => {
  ...
  return raw.flatMap(item => {
    ...
    if (item.recurrence && typeof item.recurrence === 'string') {
      ...
      const times = Array.isArray(item.time) ? item.time : (item.time ? [item.time] : [])
    ...
    const date = moment(item.date, ['MM-DD-YYYY','YYYY-MM-DD']).format('YYYY-MM-DD')
    const times = Array.isArray(item.time) ? item.time : (item.time ? [item.time] : [])
    ...
    return times.map((t, idx) => {
      ...
      const start = moment(`${date} ${from}`, ['YYYY-MM-DD hh:mm A','YYYY-MM-DD HH:mm']).toISOString()
      const end = moment(`${date} ${to}`, ['YYYY-MM-DD hh:mm A','YYYY-MM-DD HH:mm']).toISOString()
...
const formToPayload = (f) => {
  ...
  const timeObjects = f.times.map(timeStr => {
...
const form = ref({ id: null, date: '', times: [], notes: '', recurring: false, recurrence_days: [] })
...
const fetchSchedules = async () => {
  ...
  try {
    ...
    toast.value = { show: true, message: e?.message || 'Failed to load schedules', type: 'error' }
  } finally {
    loading.value = false
  }
}
...
const createSchedule = async () => {
  if (!form.value.date || !form.value.times.length) {
    toast.value = { show: true, message: 'Choose date and at least one time', type: 'error' }
    return
  }
  ...
  try {
    ...
    toast.value = { 
      show: true, 
      message: e?.data?.message || e?.statusMessage || e?.message || 'Create failed', 
      type: 'error' 
    }
  } finally {
...
const updateSchedule = async () => {
  ...
  isSubmitting.value = true
  try {
    await $fetch(`${endpoint.value}/api/library/schedule/booking/${form.value.id}/update/`, { method: 'PUT', body: formToPayload(form.value) })
    closeModal()
    await fetchSchedules()
    toast.value = { show: true, message: 'Schedule updated', type: 'success' }
  } catch (e) {
    toast.value = { show: true, message: e?.data?.message || e?.message || 'Update failed', type: 'error' }
  } finally {
    isSubmitting.value = false
  }
}
...
const deleteSchedule = async () => {
  ...
  isSubmitting.value = true
  try {
    await $fetch(`${endpoint.value}/api/library/schedule/booking/${form.value.id}/delete/`, { method: 'DELETE' })
    showDeleteConfirm.value = false
    await fetchSchedules()
    toast.value = { show: true, message: 'Schedule removed', type: 'success' }
  } catch (e) {
    toast.value = { show: true, message: e?.data?.message || e?.message || 'Delete failed', type: 'error' }
  } finally {
    isSubmitting.value = false
  }
}
...
const handleEventDrop = async (dropInfo) => {
  ...
  try {
    await $fetch(`${endpoint.value}/api/library/schedule/booking/${orig.id}/update/`, { method: 'PUT', body: payload })
    toast.value = { show: true, message: 'Schedule moved', type: 'success' }
    await fetchSchedules()
  } catch (e) {
    toast.value = { show: true, message: e?.message || 'Move failed', type: 'error' }
    dropInfo.revert()
  }
}
...
const handleEventResize = async (resizeInfo) => {
  ...
  const s = moment(evt.start).format('hh:mm A')
  const e = moment(evt.end).format('hh:mm A')
  const payload = { date: moment(evt.start).format('MM-DD-YYYY'), time: [`${s} - ${e}`] }
  ...
  try {
    ...
    toast.value = { show: true, message: e?.message || 'Resize failed', type: 'error' }
    resizeInfo.revert()
  }
}
...
const openCreateFromButton = () => {
  ...
  form.value = { id: null, date: moment().format('YYYY-MM-DD'), times: [], notes: '', recurring: false, recurrence_days: [] }
...
const closeModal = () => {
  ...
  isEditing.value = false
  form.value = { id: null, date: '', times: [], notes: '', recurring: false, recurrence_days: [] }
}
...
/* ----------------------- Computeds ----------------------- */
const upcoming = computed(() => schedules.value.slice().sort((a,b) => moment(a.date, ['MM-DD-YYYY','YYYY-MM-DD']).diff(moment(b.date, ['MM-DD-YYYY','YYYY-MM-DD']))))
...
onMounted(async () => {
  try {
    ...
    let rrule = null
    try { rrule = (await import('@fullcalendar/rrule')).default } catch (e) { /* optional */ }

    FullCalendar.value = fcModule.default || fcModule
    calendarPlugins.value = rrule ? [dayGrid, timeGrid, interaction, rrule] : [dayGrid, timeGrid, interaction]
    ...
    toast.value = { show: true, message: 'Calendar failed to load', type: 'error' }
  }
...
<style scoped>
.bg-green-800 { background-color: #096b2e; }
