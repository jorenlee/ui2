      ...
      <section class="animate-[fadeIn_0.4s_ease-out] relative">
            ...
            <div class="p-5">
              ...
              <form @submit.prevent="validateVoter">
                ...
                <button
                  type="submit"
                  :disabled="
                    loading || !voterIdInput.trim() || !emailInput.trim()
                  "
              ...
              <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                >
                  ...
                  >
                    <img
                      :src="
                        getProfileImageUrl(c.student_candidate_profile_image)
                      "
                      alt="Profile"
        ...
        <div v-else class="animate-[fadeIn_0.4s_ease-out]">
          ...
          >
            <!-- TABS HEADER -->
            <div
              class="flex flex-wrap justify-center border-b border-slate-200 mb-8 pb-px gap-2 sm:gap-6"
            >
              <!-- USG -->
              <button
            ...
            <!-- USG CONTENT -->
            <div
              v-if="activeSection === 'usg'"
              class="animate-[fadeIn_0.3s_ease-out]"
            >
              <div class="border-b-2 border-slate-100 pb-2 mb-6">
                <h3
                  class="text-xl font-bold text-slate-800 uppercase tracking-wide flex items-center gap-2"
                >
              ...
              <div v-if="Object.keys(usgCandidatesGrouped).length > 0">
                ...
                >
                  <h4
                    class="text-lg font-bold text-green-800 bg-green-50/50 px-4 py-2 rounded-lg border-l-4 border-green-600 mb-6 flex items-center gap-2"
                  >
                    {{ position }}
                    <span
                      v-if="isSingleSelectPosition(position)"
                      class="text-[10px] font-bold text-green-700 bg-green-100 px-2 py-0.5 rounded-full normal-case tracking-normal"
                    >
                      Select 1
                    </span>
                  </h4>
                  ...
                  >
                    ...
                    >
                      <input
                        ...
                        :name="
                          isSingleSelectPosition(position)
                            ? getGroupKey('usg', position)
                            : undefined
                        "
                        :value="c.id"
                        v-model="selectedCandidates"
                        @change="
                          onCandidateToggle('usg', position, group, c.id)
                        "
                        class="hidden"
                      />
                      ...
                      >
                        <img
                          ...
                          :src="
                            getProfileImageUrl(
                              c.student_candidate_profile_image,
                            )
                          "
                          alt="Profile"
                    ...
                    >
                      <div
                        class="relative w-full aspect-square mb-4 rounded-full overflow-hidden shadow-inner bg-slate-100 flex items-center justify-center shrink-0"
                      >
                        <svg
                          xmlns="http://www.w3.org/2000/svg"
                      ...
                      <div class="w-full flex flex-col items-center">
                        <span
                          class="text-[10px] font-bold text-slate-500 uppercase tracking-widest bg-slate-200/70 px-3 py-1 rounded-full mb-2"
                        >
                          {{ position }}
                        </span>
                        <h5
                          class="font-extrabold text-slate-700 text-base leading-tight mb-1"
                        >
                          Abstain
                        </h5>
                        <p
              ...
              >
                <h4 class="text-lg font-bold text-slate-600">
                  No USG candidates available.
                </h4>
              </div>
            </div>
            ...
            >
              <div class="border-b-2 border-slate-100 pb-2 mb-6">
                ...
                >
                  ...
                  <span
                    class="text-xs font-bold bg-emerald-100 text-emerald-700 px-3 py-1 rounded-full normal-case tracking-normal"
                  >
                    {{ getVoterLocalCategory(currentVoter) }}
                  </span>
                </h3>
                <p class="text-xs text-slate-400 mt-1 font-semibold">
                  Exclusively visible and restricted to students belonging to:
                  {{ getVoterLocalCategory(currentVoter) }}.
                </p>
              </div>
              ...
              <div v-if="Object.keys(localCandidatesGrouped).length > 0">
                ...
                >
                  <h4
                    class="text-lg font-bold text-emerald-800 bg-emerald-50/50 px-4 py-2 rounded-lg border-l-4 border-emerald-600 mb-6 flex items-center gap-2"
                  >
                    {{ position }}
                    <span
                      v-if="isSingleSelectPosition(position)"
                      class="text-[10px] font-bold text-emerald-700 bg-emerald-100 px-2 py-0.5 rounded-full normal-case tracking-normal"
                    >
                      Select 1
                    </span>
                  </h4>
                  ...
                  >
                    ...
                    >
                      <input
                        ...
                        :name="
                          isSingleSelectPosition(position)
                            ? getGroupKey('local', position)
                            : undefined
                        "
                        :value="c.id"
                        v-model="selectedCandidates"
                        @change="
                          onCandidateToggle('local', position, group, c.id)
                        "
                        class="hidden"
                      />
              ...
              >
                <h4 class="text-lg font-bold text-slate-600">
                  No Local Council candidates available for your college.
                </h4>
              </div>
            </div>
            ...
            >
              <div class="border-b-2 border-slate-100 pb-2 mb-6">
                ...
                >
                  ...
                  <span
                    v-if="getVoterABO(currentVoter)"
                    class="text-xs font-bold bg-indigo-100 text-indigo-700 px-3 py-1 rounded-full normal-case tracking-normal"
                  >
                ...
                <p class="text-xs text-slate-400 mt-1 font-semibold">
                  Exclusively visible and restricted to students belonging to
                  the organization:
                  {{ getVoterABO(currentVoter) || "Not Mapped" }}.
                </p>
              </div>
              ...
              <div v-if="Object.keys(aboCandidatesGrouped).length > 0">
                ...
                >
                  <h4
                    class="text-lg font-bold text-indigo-800 bg-indigo-50/50 px-4 py-2 rounded-lg border-l-4 border-indigo-600 mb-6 flex items-center gap-2"
                  >
                    {{ position }}
                    <span
                      v-if="isSingleSelectPosition(position)"
                      class="text-[10px] font-bold text-indigo-700 bg-indigo-100 px-2 py-0.5 rounded-full normal-case tracking-normal"
                    >
                      Select 1
                    </span>
                  </h4>
                  ...
                  >
                    ...
                    >
                      <input
                        ...
                        :name="
                          isSingleSelectPosition(position)
                            ? getGroupKey('abo', position)
                            : undefined
                        "
                        :value="c.id"
                        v-model="selectedCandidates"
                        @change="
                          onCandidateToggle('abo', position, group, c.id)
                        "
                        class="hidden"
                      />
                      ...
                      <div class="w-full flex flex-col items-center">
                        <span
                          class="text-[10px] font-bold text-indigo-700 uppercase tracking-widest bg-indigo-100/70 px-3 py-1 rounded-full mb-2"
                        >
                          {{ c.title_position }}
                        </span>
                        <h5
              ...
              >
                <h4 class="text-lg font-bold text-slate-600">
                  No ABO candidates available matching your organization
                  profile.
                </h4>
              </div>
            </div>
            ...
            <div class="text-center mt-16 pt-8 border-t border-slate-200">
              <button
                @click="castVote"
                :disabled="loading || isBallotEmpty || !hasVisitedAllTabs"
...
const hasVisitedAllTabs = computed(() => {
  return (
    visitedTabs.value.usg && visitedTabs.value.local && visitedTabs.value.abo
  );
});
...
// Positions where a voter may only pick ONE candidate (President / Vice
// President, Governor / Vice Governor). Matched by substring so wording
// variations (e.g. "USG President", "Governor for Internal Affairs") are
// still recognized regardless of exact phrasing.
const isSingleSelectPosition = (position) => {
  const p = (position || "").trim().toLowerCase();
  return p.includes("president") || p.includes("governor");
};
...
// Fired whenever a candidate checkbox/radio is toggled. Always clears any
// abstain flag on the group. Additionally, for single-select positions
// (President/Vice President, Governor/Vice Governor) enforces that only
// the just-picked candidate remains selected within that same position
// group, so a voter can never end up with two candidates checked for a
// position that only has one seat.
...
const isBallotEmpty = computed(() => {
  return (
    selectedCandidates.value.length === 0 && abstainedGroups.value.size === 0
  );
});
...
const castVote = async () => {
  ...
  try {
    ...
    if (res.ok) {
      showToast(
        "Vote cast successfully! A ballot receipt has been sent to your email.",
        "success",
      );
...
const usgCandidatesGrouped = computed(() => {
  ...
  candidates.value.forEach((c) => {
    if (
      c.category &&
      (c.category.toLowerCase() === "all colleges" ||
        c.category.toUpperCase() === "LSU-USG")
    ) {
      const posKey = (c.title_position || "").trim();
      if (!groups[posKey]) {
        groups[posKey] = [];
      }
      groups[posKey].push(c);
    }
  });
...
const getVoterLocalCategory = (voter) => {
  ...
  const cte = ["CTE", "COLLEGE OF TEACHER EDUCATION", "TEACHER EDUCATION"];
  if (cte.some((kw) => college.includes(kw))) {
    return "SC-CTE";
  }

  // College: CON — College of Nursing
  const con = ["CON", "COLLEGE OF NURSING", "NURSING"];
...
const localCandidatesGrouped = computed(() => {
  ...
  candidates.value.forEach((c) => {
    if (
      c.category &&
      c.category.toUpperCase() === targetCategory.toUpperCase()
    ) {
      const posKey = (c.title_position || "").trim();
      if (!groups[posKey]) {
        groups[posKey] = [];
