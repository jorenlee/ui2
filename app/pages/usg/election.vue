<template>
  <div>
    ...
    <!-- Header -->
    <header class="w-full bg-white border-b border-gray-100 shadow-sm px-4 sm:px-6 lg:px-8 py-4">
      <div class="max-w-7xl mx-auto flex flex-col lg:flex-row items-center justify-between gap-3">
        ...
        <img
          ...
          alt="La Salle University" class="h-10 sm:h-12 w-auto object-contain" />
        ...
        <div
          ...
          <span class="lg:inline-block hidden w-2 h-2 rounded-full bg-green-500 animate-pulse flex-shrink-0"></span>
      ...
      <div v-else class="animate-[fadeIn_0.4s_ease-out]">
        ...
        ">
          ...
          <div class="flex flex-wrap justify-center border-b border-slate-200 mb-8 pb-px gap-2 sm:gap-6">
            <button @click="activeSection = 'usg'" type="button" :class="[
            ...
            <button @click="activeSection = 'local'" type="button" :class="[
              'px-6 py-3 font-extrabold text-base sm:text-lg border-b-4 transition-all duration-300 relative flex items-center gap-2',
              activeSection === 'local'
            ...
            <button @click="activeSection = 'abo'" type="button" :class="[
              'px-6 py-3 font-extrabold text-base sm:text-lg border-b-4 transition-all duration-300 relative flex items-center gap-2',
              activeSection === 'abo'
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 xl:grid-cols-4 gap-6">
                  <label v-for="c in group" :key="c.id" :class="[
                    ...
                    <input type="checkbox" :value="c.id" v-model="selectedCandidates"
                      @change="clearAbstain('usg', position)" class="hidden" />
              ...
              <div v-for="(group, position) in localCandidatesGrouped" :key="position" class="mb-10">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 xl:grid-cols-4 gap-6">
                  <label v-for="c in group" :key="c.id" :class="[
                    ...
                    <input type="checkbox" :value="c.id" v-model="selectedCandidates"
                      @change="clearAbstain('local', position)" class="hidden" />
          ...
          <div v-if="activeSection === 'abo'" class="animate-[fadeIn_0.3s_ease-out]">
            <div class="border-b-2 border-slate-100 pb-2 mb-6">
              ...
              <p class="text-xs text-slate-400 mt-1 font-semibold">
                Exclusively visible and restricted to students belonging to the organization: {{
                  getVoterABO(currentVoter) ||
                'Not Mapped' }}.
              </p>
            </div>
            ...
            <div v-if="Object.keys(aboCandidatesGrouped).length > 0">
              <div v-for="(group, position) in aboCandidatesGrouped" :key="position" class="mb-10">
                <h4
                  class="text-lg font-bold text-indigo-800 bg-indigo-50/50 px-4 py-2 rounded-lg border-l-4 border-indigo-600 mb-6">
                  {{ position }}
                </h4>
                <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 xl:grid-cols-4 gap-6">
                  <label v-for="c in group" :key="c.id" :class="[
                    ...
                    <input type="checkbox" :value="c.id" v-model="selectedCandidates"
                      @change="clearAbstain('abo', position)" class="hidden" />
            ...
            <div v-else class="text-center py-12 bg-slate-50 rounded-2xl border-2 border-dashed border-slate-200">
              <h4 class="text-lg font-bold text-slate-600">No ABO candidates available matching your organization
                profile.
              </h4>
            </div>
          </div>
          ...
          <div class="text-center mt-16 pt-8 border-t border-slate-200">
            <button @click="castVote" :disabled="loading || isBallotEmpty"
