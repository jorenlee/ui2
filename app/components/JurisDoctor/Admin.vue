<template>
  <div class="min-h-screen bg-gray-50 font-sans">
    ...
    <div class="w-11/12 mx-auto space-y-8">
          ...
          <div class="relative z-10 flex flex-col lg:flex-row justify-between items-start lg:items-center gap-4">
            <div>
              ...
              <p class="mt-2 text-green-100">
                Manage questionnaires, review essays, and monitor applicant examinations.
              </p>
            </div>

            <div class="bg-white/10 backdrop-blur-md border border-white/20 rounded-2xl px-5 py-3 text-white">
              <div class="text-xs uppercase tracking-widest opacity-80">
                Administration Panel
              </div>
              <div class="font-bold">
                Law Admissions System
              </div>
            </div>
          </div>
        </div>
      ...
      <div class="lg:flex w-full gap-x-4">
        <!-- Statistics -->
        <div class="space-y-4 w-full">
          ...
          <div
            ...
            <div class="flex items-center justify-between">
              ...
              <div class="w-14 h-14 rounded-2xl bg-green-100 text-[#006B3F] flex items-center justify-center">
        ...
        <div class="w-full bg-white rounded-3xl shadow-sm border border-gray-100 overflow-hidden">
          ...
          <div class="px-8 py-5 border-b border-gray-100">
            <h2 class="font-black text-gray-800 text-lg flex items-center gap-3">
              <div class="w-10 h-10 rounded-xl bg-green-100 text-[#006B3F] flex items-center justify-center">
          ...
          <div class="p-8">
            ...
            <div class="mb-8">
              ...
              <button
                ...
                <div class="w-12 h-12 rounded-xl bg-[#006B3F] text-white flex items-center justify-center">
                  <i class="fas fa-file-csv"></i>
                </div>

                <div class="text-left">
                  <div class="font-black text-[#006B3F]">
                    CSV Upload
                  </div>
            ...
            <div
              ...
              <input type="file" :accept="fileAccept" @change="onFileChange"
        ...
        <div class="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
          <div class="p-6 border-b border-gray-100 flex flex-wrap items-center justify-between gap-4">
            ...
            <div class="flex items-center gap-4">
              <button v-if="selectedAttemptIds.length > 0" @click="deleteSelectedAttempts"
                ...
                <i class="fas fa-trash-alt"></i> Delete Selected ({{ selectedAttemptIds.length }})
              </button>
              <button @click="fetchAttempts" :disabled="loadingAttempts"
            ...
            <table class="w-full text-left border-collapse">
              ...
              <tbody class="divide-y divide-gray-100 text-sm text-gray-600">
                <tr v-for="attempt in attempts" :key="attempt.id" class="hover:bg-gray-50/50 transition"
                  :class="{ 'bg-blue-50/20': selectedAttemptIds.includes(attempt.id) }">
                  <td class="p-4 pl-6 text-center w-12">
                    <input type="checkbox" v-model="selectedAttemptIds" :value="attempt.id"
                      class="w-4 h-4 rounded text-blue-600 border-gray-300 focus:ring-blue-500 cursor-pointer" />
                  </td>
                  <td class="p-4">
                    <div class="font-extrabold text-gray-800 uppercase">{{ attempt.fullname }}</div>
                    <div class="text-[11px] text-gray-400 font-medium">{{ attempt.email }}</div>
                    <div class="text-[10px] text-gray-400 font-medium mt-0.5">Taken: {{ formatDate(attempt.created_at)
                      }}</div>
                  </td>
                  <td class="p-4">
                    <div class="font-bold text-gray-800">{{ attempt.score }} / {{ attempt.total_questions }}</div>
                    <div class="text-[11px] font-medium mt-0.5"
                      :class="attempt.percentage >= 75 ? 'text-green-600' : 'text-red-500'">
                  ...
                  <td class="p-4">
                    <span class="px-2 py-0.5 rounded text-[10px] font-bold uppercase tracking-wider" :class="attempt.score_notified
                  ...
                  <td class="p-4 pr-6 text-right">
                    <div class="flex items-center justify-end gap-2">
                      <button @click="openGradeModal(attempt)"
                        class="px-3 py-1.5 rounded-lg border text-xs font-bold transition shadow-sm" :class="attempt.score_notified
                          ? 'bg-white hover:bg-gray-50 border-gray-200 text-gray-600'
                          : 'bg-blue-600 hover:bg-blue-700 border-blue-600 text-white'">
                        {{ attempt.score_notified ? 'View Detail' : 'Grade Essay' }}
                      </button>
                      <button @click="deleteAttempt(attempt.id, attempt.fullname)" type="button"
      ...
      <div class="bg-white p-6 rounded-2xl border border-gray-100 shadow-sm space-y-5">
        ...
        <div v-if="showAddForm" class="space-y-4">
          ...
          <div class="flex items-center gap-3 p-3 rounded-xl bg-blue-50/50 border border-blue-100">
            <input type="checkbox" id="isEssayCheck" v-model="newQuestion.is_essay"
          ...
          <!-- Passage -->
          <div>
            <label class="block text-xs font-bold text-gray-500 uppercase tracking-wider mb-1.5">Passage <span
                class="normal-case font-medium text-gray-400">(optional)</span></label>
            <textarea v-model="newQuestion.passage" rows="3" placeholder="Enter reading passage if applicable..."
              class="w-full px-3 py-2.5 rounded-xl border border-gray-200 text-sm text-gray-700 font-medium focus:outline-none focus:ring-2 focus:ring-[#006B3F] bg-gray-50 resize-y"></textarea>
          </div>

          <!-- Question Text -->
          <div>
            <label class="block text-xs font-bold text-gray-500 uppercase tracking-wider mb-1.5">Question <span
                class="text-red-500">*</span></label>
            <textarea v-model="newQuestion.question" rows="3" placeholder="Enter the question text..."
              class="w-full px-3 py-2.5 rounded-xl border border-gray-200 text-sm text-gray-700 font-medium focus:outline-none focus:ring-2 focus:ring-[#006B3F] bg-gray-50 resize-y"></textarea>
          </div>

          <!-- MCQ Fields (hidden when essay) -->
          <div v-if="!newQuestion.is_essay" class="space-y-3 pt-1">
            <p class="text-[10px] font-extrabold text-gray-400 uppercase tracking-widest border-b pb-1">Multiple Choice
              Options</p>

            <!-- Correct Answer -->
            <div>
              <label class="block text-xs font-bold text-green-700 uppercase tracking-wider mb-1">Correct Answer <span
                  class="text-red-500">*</span></label>
              <input type="text" v-model="newQuestion.correct_answer" placeholder="Type the correct answer..."
            ...
            <div>
              ...
              <input type="text" v-model="newQuestion.option_1" placeholder="Enter option 1..."
            ...
            <div>
              ...
              <input type="text" v-model="newQuestion.option_2" placeholder="Enter option 2..."
                class="w-full px-3 py-2.5 rounded-xl border border-gray-200 bg-gray-50 text-sm text-gray-700 font-medium focus:outline-none focus:ring-2 focus:ring-[#006B3F]" />
            </div>
            ...
            <div>
              ...
              <input type="text" v-model="newQuestion.option_3" placeholder="Enter option 3..."
                class="w-full px-3 py-2.5 rounded-xl border border-gray-200 bg-gray-50 text-sm text-gray-700 font-medium focus:outline-none focus:ring-2 focus:ring-[#006B3F]" />
            </div>
          </div>

          <!-- Add Form Alerts -->
          <div v-if="addQuestionError"
            class="p-3 bg-red-50 border border-red-100 text-red-700 text-xs font-bold rounded-lg">
            {{ addQuestionError }}
          </div>
          <div v-if="addQuestionSuccess"
            class="p-3 bg-green-50 border border-green-100 text-green-700 text-xs font-bold rounded-lg">
            {{ addQuestionSuccess }}
          </div>

          <!-- Submit Button -->
          <button @click="submitNewQuestion" :disabled="addingQuestion"
      ...
      <div class="bg-white p-6 md:p-8 rounded-2xl border border-gray-100 shadow-sm mt-8">
        <h2
          ...
          <div class="flex items-center gap-4">
            <button v-if="questions.length > 0" @click="clearAllQuestions" type="button"
              class="text-xs font-extrabold text-red-600 hover:text-red-800 hover:underline flex items-center gap-1">
              <i class="fas fa-trash-alt"></i> Clear All
            </button>
            <button @click="fetchQuestions" :disabled="loadingQuestions"
          ...
          <div v-for="(q, idx) in questions" :key="q.id"
            ...
            <div>
              <div class="flex items-center gap-2 mb-2">
                ...
                <button @click="deleteQuestion(q.id, idx + 1)" type="button"
                  class="ml-auto text-gray-400 hover:text-red-600 transition text-xs" title="Delete Question">
                  <i class="fas fa-trash-alt"></i>
                </button>
              </div>
            ...
            <div v-if="!q.is_essay" class="grid grid-cols-1 md:grid-cols-2 gap-4 text-xs pl-1">
              ...
              <div @click="setCorrectAnswer(q.id, q.correct_answer)"
                ...
                <div class="flex flex-col">
                  <span class="text-[9px] font-black text-green-700 uppercase tracking-wider">Correct Answer (Click to
                    Set):</span>
              ...
              <!-- Option 1 block -->
              <div @click="setCorrectAnswer(q.id, q.option_1)"
              ...
              <!-- Option 2 block -->
              <div @click="setCorrectAnswer(q.id, q.option_2)"
                class="flex items-center gap-3 p-3.5 rounded-xl border-2 border-gray-200 bg-gray-50/50 cursor-pointer hover:bg-green-50/20 hover:border-green-300 transition select-none">
                <div
                  class="w-4 h-4 rounded-full border-2 border-gray-300 flex items-center justify-center bg-white shrink-0">
                </div>
              ...
              <!-- Option 3 block -->
              <div @click="setCorrectAnswer(q.id, q.option_3)"
                class="flex items-center gap-3 p-3.5 rounded-xl border-2 border-gray-200 bg-gray-50/50 cursor-pointer hover:bg-green-50/20 hover:border-green-300 transition select-none">
                <div
                  class="w-4 h-4 rounded-full border-2 border-gray-300 flex items-center justify-center bg-white shrink-0">
                </div>
            ...
            <div v-else
              ...
              <i class="fas fa-info-circle text-blue-500"></i> Essay Prompt: No fixed options. Graded manually by
              examiner.
            </div>
    ...
    <div v-if="showGradeModal"
      ...
      <div
        ...
        <div v-if="selectedAttempt" class="space-y-5">
          ...
          <div>
            <label class="block text-xs font-bold text-gray-400 uppercase tracking-wider mb-1">Selected Essay
              Prompt:</label>
            <div
          ...
          <div class="space-y-2 pt-2 border-t">
            <div class="flex items-center justify-between">
              <label class="block text-xs font-extrabold text-gray-800 uppercase tracking-wider">Assign Essay Score (0 -
                100):</label>
              <span v-if="selectedAttempt.score_notified"
                class="text-[10px] bg-green-100 text-green-800 font-bold px-2 py-0.5 rounded">
                Already Graded & Notified
              </span>
            </div>
            <input type="number" v-model="inputEssayScore" min="0" max="100" :disabled="selectedAttempt.score_notified"
          ...
          <div class="flex items-center justify-between pt-4 border-t">
            ...
            <button @click="submitGradeAndNotify" :disabled="grading || selectedAttempt.score_notified"
...
const isAllSelected = computed(() => {
  return attempts.value.length > 0 && selectedAttemptIds.value.length === attempts.value.length
})
...
watch(attempts, (newAttempts) => {
  ...
  selectedAttemptIds.value = selectedAttemptIds.value.filter(id => attemptIds.includes(id))
})
...
const deleteSelectedAttempts = async () => {
  ...
  if (!confirm(`Are you sure you want to delete the ${count} selected exam attempt(s)? This action cannot be undone.`)) {
    return
  }
  ...
  try {
    ...
    if (failedCount === 0) {
      alert(`Successfully deleted all ${deletedCount} selected exam attempt(s).`)
    } else {
      alert(`Deleted ${deletedCount} attempt(s) successfully. Failed to delete ${failedCount} attempt(s).`)
    }
