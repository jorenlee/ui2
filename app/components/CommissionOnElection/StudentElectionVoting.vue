<template>
 <div>
    ...
    <div v-if="toast.show" :class="[
      ...
      <button @click="toast.show = false" class="ml-auto text-xl opacity-50 hover:opacity-100 leading-none">
        &times;
      </button>
    </div>
        ...
        <div class="p-5 sm:p-6 overflow-y-auto">
          ...
          <template v-for="section in [
            ...
            <div v-if="section.rows.length" class="mb-4">
              <h4 class="text-xs font-bold uppercase tracking-widest text-slate-400 mb-2">
                {{ section.label }}
              </h4>
              <div v-for="(r, i) in section.rows" :key="section.key + '-' + i"
    ...
    <div class="w-full max-w-6xl mx-auto pt-5">
      ...
      <div class="text-center">
        ...
        <h1 class="lg:text-4xl text-2xl font-bold text-slate-900 tracking-tight uppercase pb-3">
    ...
    <div v-if="!currentVoter" class="flex justify-center px-4">
      <div class="w-full sm:w-4/5 md:w-1/2 lg:w-1/3 xl:w-3/12 bg-white border border-slate-200 rounded-2xl shadow-lg overflow-hidden">
        ...
        <div class="p-5">
          <!-- Icon -->
          <div class="w-16 h-16 rounded-2xl bg-green-50 text-[#087830] flex items-center justify-center mx-auto mb-6">
          ...
          <form @submit.prevent="validateVoter">
            <!-- Email -->
            <div class="mb-6">
              <input v-model="emailInput" type="email" placeholder="Enter your University Email"
            ...
            <!-- LSU ID -->
            <div class="mb-8">
              <input v-model="voterIdInput" type="text" placeholder="Enter your LSU ID Number"
                class="w-full border-b-2 lg:text-sm text-xs shadow border-green-800 px-4 py-3 focus:border-[#087830] focus:ring-4 focus:ring-green-100 outline-none transition" />
            </div>

            <!-- Button -->

            <button type="submit" :disabled="loading || !voterIdInput.trim() || !emailInput.trim()"
      ...
      ">
        ...
        <div class="flex flex-nowrap sm:flex-wrap justify-start sm:justify-center overflow-x-auto sm:overflow-visible border-b border-slate-200 mb-8 pb-px gap-2 sm:gap-6 -mx-4 px-4 sm:mx-0 sm:px-0"
          ...
          <button @click="changeSection('local')" type="button" :class="[
            ...
            <span v-if="selectedLocalCount > 0"
          ...
          <div v-if="Object.keys(usgCandidatesGrouped).length > 0">
            <div v-for="(group, position) in usgCandidatesGrouped" :key="position" class="mb-10">
              <h4
                ...
                {{ position }}
                <span v-if="isSingleSelectPosition(position)"
...
/>
                  ...
                  <div class="relative w-full aspect-square mb-4 overflow-hidden shadow-inner bg-slate-100 shrink-0">
                    <img v-if="c.student_candidate_profile_image"
                      :src="getProfileImageUrl(c.student_candidate_profile_image)" alt="Profile"
                ...
                <!-- Abstain option -->
                <button type="button" @click="toggleAbstain('usg', position, group)" :class="[
          ...
          <div v-if="Object.keys(localCandidatesGrouped).length > 0">
            <div v-for="(group, position) in localCandidatesGrouped" :key="position" class="mb-10">
              ...
              <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 sm:gap-6">
                ...
                <!-- Abstain option -->
                <button type="button" @click="toggleAbstain('local', position, group)" :class="[
                  'relative flex flex-col items-center justify-center p-5 border-2 border-dashed rounded-2xl transition-all duration-300 hover:-translate-y-1 text-center bg-white shadow-sm',
        ...
        <div v-if="activeSection === 'abo'" class="animate-[fadeIn_0.3s_ease-out]">
          <div class="border-b-2 border-slate-100 pb-2 mb-6">
            <h3 class="text-xl font-bold text-slate-800 uppercase tracking-wide flex items-center gap-2">
              ...
              <span v-if="getVoterABO(currentVoter)"
            ...
            <p class="text-xs text-slate-400 mt-1 font-semibold">
              Exclusively visible and restricted to students belonging to the organization: {{ getVoterABO(currentVoter)
                ||
                'Not Mapped' }}.
            </p>
          </div>
          ...
          <div v-if="Object.keys(aboCandidatesGrouped).length > 0">
            <div v-for="(group, position) in aboCandidatesGrouped" :key="position" class="mb-10">
              <h4
              ...
              <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 sm:gap-6">
                ...
                <!-- Abstain option -->
                <button type="button" @click="toggleAbstain('abo', position, group)" :class="[
                  'relative flex flex-col items-center justify-center p-5 border-2 border-dashed rounded-2xl transition-all duration-300 hover:-translate-y-1 text-center bg-white shadow-sm',
          ...
          <div v-else class="text-center py-12 bg-slate-50 rounded-2xl border-2 border-dashed border-slate-200">
            <h4 class="text-lg font-bold text-slate-600">No ABO candidates available matching your organization profile.
            </h4>
          </div>
        </div>
        ...
        <div class="text-center mt-16 pt-8 border-t border-slate-200">
          <button @click="castVote" :disabled="loading ||
            isBallotEmpty ||
            !hasVisitedAllTabs
            "
