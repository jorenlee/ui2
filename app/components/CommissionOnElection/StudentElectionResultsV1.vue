    ...
    <div class="mb-14">
      ...
      <div class="mb-8">
        <div class="flex items-center gap-4 mb-4">
          ...
          <span class="text-xs font-bold text-slate-400 bg-slate-100 rounded-full px-3 py-1 whitespace-nowrap">All students</span>
          <div class="flex-1 h-px bg-slate-200"></div>
        </div>
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-6 max-w-2xl">
          <div class="bg-slate-50 border border-slate-200 rounded-xl p-6">
            <div class="text-xs font-bold text-slate-500 uppercase tracking-wider mb-1">Total Registered Voters</div>
            <div class="text-4xl font-black text-slate-800">{{ voterStats.total_voters || 0 }}</div>
          </div>
          <div class="bg-green-50 border border-green-200 rounded-xl p-6">
            <div class="text-xs font-bold text-green-700 uppercase tracking-wider mb-1">Total Votes Cast</div>
            ...
            <div class="flex justify-between items-baseline mb-4">
              ...
              <div class="text-right">
                <div class="text-2xl font-black text-emerald-600 leading-none">
                  {{ getCollegeVotedTotal(g) }} <span class="text-xs font-medium text-slate-400">/</span> {{ getCollegeTotal(g) }}
                </div>
                <div class="text-[10px] font-bold text-slate-400 uppercase tracking-wider">Voted / Reg</div>
              </div>
            </div>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-6">
          <div v-for="g in ABO_GROUPS" :key="g.key" class="bg-slate-50 border border-slate-200 rounded-xl p-6">
            <div class="flex justify-between items-baseline mb-4">
              ...
              <div class="text-right">
                <div class="text-2xl font-black text-emerald-600 leading-none">
                  {{ getAboVotedTotal(g) }} <span class="text-xs font-medium text-slate-400">/</span> {{ getAboTotal(g) }}
                </div>
                <div class="text-[10px] font-bold text-slate-400 uppercase tracking-wider">Voted / Reg</div>
              </div>
            </div>
    ...
    <div v-if="groupedCandidates.length > 0">
      <div v-for="group in groupedCandidates" :key="group.key" class="mb-16 last:mb-0">
        ...
        <div class="flex items-center gap-4 mb-6">
          <h2 class="text-xl font-extrabold text-slate-900 tracking-widest whitespace-nowrap">{{ group.label }}</h2>
          <span class="text-xs font-bold text-slate-400 bg-slate-100 rounded-full px-3 py-1 whitespace-nowrap">
          ...
          <div v-for="c in group.candidates" :key="c.id"
            ...
            <div class="mt-6">
              <div class="flex justify-between items-baseline mb-1">
                <span class="text-4xl font-black tracking-tight text-green-600">{{ getCandidateVoteStats(c).percentString }}%</span>
                <span class="text-xs font-bold text-slate-400">{{ getCandidateVoteStats(c).votes }} {{ getCandidateVoteStats(c).votes === 1 ? 'vote' : 'votes' }}</span>
              </div>
              <div class="w-full bg-slate-200/70 h-3 rounded-full overflow-hidden mb-2">
                <div class="h-full rounded-full bg-green-600 transition-all duration-500 ease-out" :style="{ width: `${getCandidateVoteStats(c).percent}%` }"></div>
              </div>
              <div class="text-[11px] font-bold text-slate-400 uppercase tracking-wider">{{ getCandidateVoteStats(c).label }}</div>
            </div>
          </div>
        </div>
        ...
        <div v-else class="space-y-10">
          <div v-for="sg in group.subgroups" :key="sg.key">
            <div class="flex items-center gap-3 mb-4">
              <h3 class="text-sm font-extrabold text-slate-600 tracking-widest uppercase whitespace-nowrap">{{ sg.label }}</h3>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-6">
              <div v-for="c in sg.candidates" :key="c.id"
                ...
                <div class="mt-6">
                  <div class="flex justify-between items-baseline mb-1">
                    <span class="text-4xl font-black tracking-tight text-emerald-600">{{ getCandidateVoteStats(c).percentString }}%</span>
                    <span class="text-xs font-bold text-slate-400">{{ getCandidateVoteStats(c).votes }} {{ getCandidateVoteStats(c).votes === 1 ? 'vote' : 'votes' }}</span>
                  </div>
                  <div class="w-full bg-slate-200/70 h-3 rounded-full overflow-hidden mb-2">
                    <div class="h-full rounded-full bg-emerald-600 transition-all duration-500 ease-out" :style="{ width: `${getCandidateVoteStats(c).percent}%` }"></div>
                  </div>
                  <div class="text-[11px] font-bold text-slate-400 uppercase tracking-wider">{{ getCandidateVoteStats(c).label }}</div>
                </div>
              </div>
...
// come back as a numeric string, null, or "" â€” never trust it as a number
...
const groupedCandidates = computed(() => {
  ...
  for (const c of listWithAbstains) {
    ...
    // â”€â”€ USG â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    if (/usg|all colleges/i.test(cat)) {
      usgCandidates.push(c);
      continue;
    }

    // â”€â”€ ABO â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    // â”€â”€ COLLEGES (SC-{code} and everything else) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    // Extract the college code from the category string (e.g. "SC-CCSEA" â†’ "CCSEA").
...
const normalizeCollege = (college) => {
  ...
  // CBA â€“ covers both College of Business and College of Accountancy
  if (c === 'CBA' || c.includes('BUSINESS') || c.includes('ACCOUNTANCY')) return 'CBA';
  // CCJE
  if (c === 'CCJE' || c.includes('CRIMINAL')) return 'CCJE';
  // CCSEA â€“ covers College of Engineering and Architecture + College of Computer Studies
  ...
  // CTHM â€“ School of Tourism and Hospitality Management
...
// Academic Based Organizations â€” each ABO draws its voters from specific programs.
const ABO_GROUPS = [
  // CAS
