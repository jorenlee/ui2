...
totalAmountS1.value = donationLogsS1List.value
  .map((item) => item.amount)
  .reduce((prev, curr) => prev + curr, 0);
totalAmountRoundedS1.value = totalAmountS1.value.toFixed(2);
totalAmountRoundedFormattedS1.value = formatterS1.format(
  totalAmountRoundedS1.value,
);
...
totalAmountS2.value = donationLogsS2List.value
  .map((item) => item.amount)
  .reduce((prev, curr) => prev + curr, 0);
totalAmountRoundedS2.value = totalAmountS2.value.toFixed(2);
totalAmountRoundedFormattedS2.value = formatterS2.format(
  totalAmountRoundedS2.value,
);
...
totalAmountS3.value = donationLogsS3List.value
  .map((item) => item.amount)
  .reduce((prev, curr) => prev + curr, 0);
totalAmountRoundedS3.value = totalAmountS3.value.toFixed(2);
totalAmountRoundedFormattedS3.value = formatterS3.format(
  totalAmountRoundedS3.value,
);
...
totalAmountS4.value = donationLogsS4List.value
  .map((item) => item.amount)
  .reduce((prev, curr) => prev + curr, 0);
totalAmountRoundedS4.value = totalAmountS4.value.toFixed(2);
totalAmountRoundedFormattedS4.value = formatterS4.format(
  totalAmountRoundedS4.value,
);
...
totalAmountALL_SI.value = donationLogsALL_SIList.value
  .map((item) => item.amount)
  .reduce((prev, curr) => prev + curr, 0);
totalAmountRoundedALL_SI.value = totalAmountALL_SI.value.toFixed(2);
totalAmountRoundedFormattedALL_SI.value = formatterALL_SI.format(
  totalAmountRoundedALL_SI.value,
);
...
onMounted(async () => {
  ...
  donationLogsSorted.value = donationLogs.value.sort(
    (b, a) => a.amount - b.amount,
  );

  percentTotalAmountS1.value =
    ((totalAmountS1.value / totalAmount.value) * 100).toFixed(2) + "%";
  percentTotalAmountS2.value =
    ((totalAmountS2.value / totalAmount.value) * 100).toFixed(2) + "%";
  percentTotalAmountS3.value =
    ((totalAmountS3.value / totalAmount.value) * 100).toFixed(2) + "%";
  percentTotalAmountS4.value =
    ((totalAmountS4.value / totalAmount.value) * 100).toFixed(2) + "%";
  percentTotalAmountGeneral.value =
    ((totalAmountALL_SI.value / totalAmount.value) * 100).toFixed(2) + "%";
...
const generatePieChart = () => {
  ...
  totalAmountCash.value = donationLogsCash.value
    .map((item) => item.amount)
    .reduce((prev, curr) => prev + curr, 0);
  totalAmountPledge.value = donationLogsPledge.value
    .map((item) => item.amount)
    .reduce((prev, curr) => prev + curr, 0);
  totalAmountBankTranfer.value = donationLogsBankTransfer.value
    .map((item) => item.amount)
    .reduce((prev, curr) => prev + curr, 0);

  degreeCashNumber.value = (totalAmountCash.value / totalAmount.value) * 100;
  degreePledgeNumber.value =
    (totalAmountPledge.value / totalAmount.value) * 100;
  degreeBankTransferNumber.value =
    (totalAmountBankTranfer.value / totalAmount.value) * 100;
  ...
  <div class="relative font-montserrat">
    ...
    <div class="lg:h-4/6 block lg:pb-10 bg-white">
      <div class="lg:shadow-lg w-11/12 mx-auto">
        ...
        <div
          class="px-3 lg:mt-5 mb-5 shadow-md lg:w-fit mx-auto text-center py-3"
        >
        ...
        <div class="px-2 lg:flex gap-10">
          ...
          <div class="lg:flex items-center w-full shadow">
            <div class="w-full">
              <h2
                class="text-center mt-4 font-bold uppercase lg:text-sm text-xs lg:py-0 pt-4"
              >
                Donations per Strategic Intent
              </h2>
              <ul
                class="lg:py-10 pt-3 lg:pb-10 pb-7 lg:px-5 px-3 w-full lg:text-sm text-xs"
              >
                <li class="lg:flex w-full items-center lg:mb-7 mb-3">
                  <span
                    class="w-[60px] whitespace-nowrap text-right lg:text-sm text-xs mr-3 font-bold"
                    >S.I. 1:</span
                  >
                  <span class="border shadow-lg flex w-full items-center">
                    <span
                      class="bg-green-400 lg:h-10 h-5 flex items-center text-center whitespace-nowrap border-green-300 lg:w-[120px] lg:text-sm text-[10px] lg:px-3 px-1 text-gray-600"
                    >
                      PHP {{ totalAmountS1 }}
                    </span>
                    <span class="flex w-full items-center">
                      <span
                        class="bg-green-400 lg:h-10 h-5 flex items-center text-center whitespace-nowrap px-3 text-gray-600 percentTotalAmountS1"
                      ></span>
                      <span
                        class="lg:ml-2 ml-auto font-bold whitespace-nowrap px-1 w-fit"
                      >
                        {{ percentTotalAmountS1 }}</span
                      >
                    </span>
                  </span>
                </li>
                <li class="lg:flex w-full items-center lg:mb-7 mb-3">
                  ...
                  <span class="border shadow-lg flex w-full items-center">
                    <span
                      class="bg-green-500 lg:h-10 h-5 flex items-center text-center whitespace-nowrap border-green-300 lg:w-[120px] lg:text-sm text-[10px] lg:px-3 px-1 text-gray-700"
                    >
                      PHP {{ totalAmountS2 }}</span
                    >
                    <span class="flex w-full items-center">
                      <span
                        class="bg-green-500 lg:h-10 h-5 flex items-center text-center whitespace-nowrap px-3 text-gray-700 percentTotalAmountS2"
                      ></span>
                      <span
                        class="lg:ml-2 ml-auto font-bold whitespace-nowrap px-1 w-fit"
                      >
                        {{ percentTotalAmountS2 }}</span
                      >
                    </span>
                  </span>
                </li>
                <li class="lg:flex w-full items-center lg:mb-7 mb-3">
                  ...
                  <span class="border shadow-lg flex w-full items-center">
                    <span
                      class="bg-green-600 lg:h-10 h-5 flex items-center text-center whitespace-nowrap border-green-300 lg:w-[120px] lg:text-sm text-[10px] lg:px-3 px-1 text-white"
                    >
                      PHP {{ totalAmountS3 }}</span
                    >
                    <span class="flex w-full items-center">
                      <span
                        class="bg-green-600 lg:h-10 h-5 flex items-center text-center whitespace-nowrap px-3 text-white percentTotalAmountS3"
                      ></span>
                      <span
                        class="lg:ml-2 ml-auto font-bold whitespace-nowrap px-1 w-fit"
                      >
                        {{ percentTotalAmountS3 }}</span
                      >
                    </span>
                  </span>
                </li>
                <li class="lg:flex w-full items-center lg:mb-7 mb-3">
                  ...
                  <span class="border shadow-lg flex w-full items-center">
                    <span
                      class="bg-green-800 lg:h-10 h-5 flex items-center text-center whitespace-nowrap border-green-300 lg:w-[120px] lg:text-sm text-[10px] lg:px-3 px-1 text-white"
                    >
                      PHP {{ totalAmountS4 }}</span
                    >
                    <span class="flex w-full items-center">
                      <span
                        class="bg-green-800 lg:h-10 h-5 flex items-center text-center whitespace-nowrap px-3 text-white percentTotalAmountS4"
                      ></span>
                      <span
                        class="lg:ml-2 ml-auto font-bold whitespace-nowrap px-1 w-fit"
                      >
                        {{ percentTotalAmountS4 }}</span
                      >
                    </span>
                  </span>
                </li>
                <li class="lg:flex w-full items-center lg:mb-7 mb-3">
                  ...
                  <span class="border shadow-lg flex w-full items-center">
                    <span
                      class="bg-green-950 lg:h-10 h-5 flex items-center text-center whitespace-nowrap border-green-300 lg:w-[120px] lg:text-sm text-[10px] lg:px-3 px-1 text-white"
                    >
                      PHP {{ totalAmountALL_SI }}</span
                    >
                    <span class="flex w-full items-center">
                      <span
                        class="bg-green-950 lg:h-10 h-5 flex items-center text-center whitespace-nowrap px-3 text-white percentTotalAmountGeneral"
                      ></span>
                      <span
                        class="lg:ml-2 ml-auto font-bold whitespace-nowrap px-1 w-fit"
                      >
                        {{ percentTotalAmountGeneral }}</span
                      >
                    </span>
                  </span>
                </li>
          ...
          <div class="w-full shadow flex items-center">
            <div class="w-full">
              <h2
                class="text-center mt-5 font-bold uppercase lg:text-sm text-xs"
              >
                Donations per Payment Mode
              </h2>
              ...
              <div class="w-fit mx-auto mt-7 mb-5">
                ...
                <div class="lg:flex mb-3">
                  <div
                    class="w-[180px] lg:text-sm text-xs text-gray-100 px-2 bg-[#0d441a] font-bold"
                    v-if="donationLogsCash"
                  >
                    Cash ({{ donationLogsCash.length }})
                  </div>
                  <div
                    class="lg:text-sm text-xs lg:ml-3 ml-2 flex"
                    v-if="donationLogsCash"
                  >
                    <span class="w-[120px]">PHP {{ totalAmountCash }}</span> :
                    {{ ((totalAmountCash / totalAmount) * 100).toFixed(2) }} %
                  </div>
                </div>
                <div class="lg:flex mb-3">
                  <div
                    class="w-[180px] lg:text-sm text-xs text-gray-100 px-2 bg-[#128db6] font-bold"
                    v-if="donationLogsPledge"
                  >
                    Pledge ({{ donationLogsPledge.length }})
                  </div>
                  <div
                    class="lg:text-sm text-xs lg:ml-3 ml-2 flex"
                    v-if="donationLogsPledge"
                  >
                    <span class="w-[120px]">PHP {{ totalAmountPledge }}</span> :
                    {{ ((totalAmountPledge / totalAmount) * 100).toFixed(2) }} %
                  </div>
                </div>
                <div class="lg:flex mb-3">
                  <div
                    class="w-[180px] lg:text-sm text-xs text-gray-100 px-2 bg-[#ffa500] font-bold"
                    v-if="donationLogsBankTransfer"
                  >
                    Bank Transfer ({{ donationLogsBankTransfer.length }})
                  </div>
                  <div
                    class="lg:text-sm text-xs lg:ml-3 ml-2 flex"
                    v-if="donationLogsBankTransfer"
                  >
                    <span class="w-[120px]"
                      >PHP {{ totalAmountBankTranfer }}</span
                    >
                    :
                    {{
                      ((totalAmountBankTranfer / totalAmount) * 100).toFixed(2)
                    }}
                    %
                  </div>
                </div>
                <div class="lg:flex mb-3">
                  <div
                    class="w-[180px] lg:text-sm text-xs font-bold text-gray-700 lg:text-left text-center"
                    v-if="donationLogs"
                  >
                    Number of Donors ({{ donationLogs.length }})
                  </div>
        ...
        <div class="lg:w-11/12 mx-auto lg:py-10 py-5 lg:text-sm text-xs">
          <h2 class="text-center bg-green-800 text-white uppercase py-2">
            Top Donors
          </h2>
          <div class="w-full shadow-lg">
            <ul>
              <li
                class="flex items-center p-1 bg-gray-300 font-bold text-green-800"
              >
              ...
              <div v-if="donationLogsSorted">
                <li
                  v-for="(j, i) in donationLogsSorted.slice(0, 5)"
                  :key="i"
                  class="flex items-center border-b p-1"
                  :class="i % 2 === 1 ? 'bg-gray-50' : ''"
                >
                  <span class="lg:w-44 w-32 text-center">{{ i + 1 }}</span>
                  <span class="lg:w-full w-8/12 lg:text-center capitalize"
                    >{{ j.name.slice(0, 3) }}*****</span
                  >
                  <span class="w-full text-center">{{ j.amount }}</span>
                  <span class="w-full text-center px-1 md:whitespace-nowrap">{{
                    j.donate_to
                  }}</span>
                </li>
              </div>
